...
void loop(){
  ...
  String jpg_path = "/picture" + String(pictureNumber) + ".jpg";
  String txt_path = "/picture" + String(pictureNumber) + ".txt";
  pictureNumber++;
  ...
  File jpg_file = fs.open(jpg_path.c_str(), FILE_WRITE);

  if(!jpg_file){
    Serial.println("Failed to open file in writing mode");
  } 
  else {
    jpg_file.write(fb->buf, fb->len); // payload (image), payload length
  ...
  File txt_file = fs.open(txt_path.c_str(), FILE_WRITE);

  if(!txt_file){
    Serial.println("Failed to open file in writing mode");
  } 
  else {
    txt_file.write(fb->buf, fb->len); // payload (image), payload length
    Serial.printf("Saved file to path: %s\n", txt_path.c_str());
